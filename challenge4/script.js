...
const cellSize = 40; // Adjust this based on maze dimensions and canvas size
...
let maze = [];
let bolletje = { x: 0, y: 0, size: 20, color: "#00ff00" }; // "Bolletje" properties

let won = false;

// Ball properties
const ballSize = 20;
const ballColor = "#ff8e0d";
let ball;
    ...
    DeviceMotionEvent.requestPermission()
      .then(permissionState => {
        if (permissionState === 'granted') {
          ...
          console.log('Toestemming voor bewegingssensoren is geweigerd. Zonder toestemming kunnen de bewegingssensoren niet worden gebruikt om het spel te spelen.');
          alert('Toestemming voor bewegingssensoren is geweigerd. Zonder toestemming kunnen de bewegingssensoren niet worden gebruikt om het spel te spelen.');
        }
      })
      .catch(console.error);
  } else {
    console.log('Deze browser ondersteunt geen toestemming voor bewegingssensoren.');
    alert('Deze browser ondersteunt geen toestemming voor bewegingssensoren.');
  }
}
...
function setup() {
  ...
  generateMaze();
  placeBolletje();
  ball = new Ball(beginX, beginY, ballSize, ballColor);
}
...
// Start generating maze from a random point
...
if (maze[y][x] === 0) continue; // Already visited
...
function moveBall() {
  ...
  if (ballCellX >= 0 && ballCellX < mazeWidth && ballCellY >= 0 && ballCellY < mazeHeight && maze[ballCellY][ballCellX] === 0) {
